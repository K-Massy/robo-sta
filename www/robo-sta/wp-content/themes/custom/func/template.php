...
function pagination($pages='', $range=4){
    ...
    if(1 != $pages){
        echo '<nav role="navigation" aria-label="ページ番号">'."";
        ...
        for($i=1; $i <= $pages; $i++){
            if(1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )){
                echo ($paged == $i) ? '<li class="page-item is-active"><span class="page-link">'.$i.'<span class="sr-only">（現在のページ）</span></span></li>'."\n" : '<li class="page-item"><a class="page-link" aria-label="'.$i.'ページ目" href="'.get_pagenum_link($i).'">'.$i.'</a></li>'."\n";
            }
        }
        ...
        if($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages){
            echo '<li class="page-item"><a class="page-link" href="'.get_pagenum_link($pages).'"> ≫ </a></li>'."\n";
        }
        echo '</ul>'."\n";
        echo '</nav>'."\n";
    }
...
function get_breadcrumbs(){
    ...
    if(!is_home()){
        // Start the UL
        echo '<nav class="breadcrumb-container" role="navigation" aria-label="現在位置">'."\n";
        ...
        if(is_category()){
            ...
            echo '<li class="breadcrumb-item is-current">'. get_category_parents( $cat, TRUE, "" ) .'</li>'."\n";
        }elseif(is_search()){
            //検索結果 ページ
            echo '<li class="breadcrumb-item is-current">検索結果</li>'."\n";
        }elseif(is_404()){
            // 404 ページ
            echo '<li class="breadcrumb-item is-current">ページが見つかりません</li>'."\n";
        }elseif(is_archive() && !is_category()){
            // アーカイブ ページ
            $parent = get_query_var('post_type');
            $object = get_post_type_object($parent)->labels->name;
            echo '<li class="breadcrumb-item is-current">'.$object.'</li>'."\n";
        }elseif(is_single()){
            // シングル ページ
            if(get_post_type() == 'post'){
                ...
                echo '<li class="breadcrumb-item is-current">'.the_title('','', FALSE) ."</li>";
            }else{
                echo '<li class="breadcrumb-item"><a href="' . home_url() . '/' . get_post_type_object(get_post_type())->name . '">' . get_post_type_object(get_post_type())->label . "</a></li>\n";
                echo '<li class="breadcrumb-item is-current">'.the_title('','', FALSE) ."</li>";
            }
        }
        elseif(is_page()){
            ...
            if($post->post_parent == 0){
                ...
                foreach ( $ancestors as $ancestor ){
                    if( $ancestor != end($ancestors) ){
                        echo '<li class="breadcrumb-item is-current"><a href="'.get_permalink($ancestor).'">'.strip_tags(apply_filters('single_post_title', get_the_title($ancestor))).'</a></li>'."\n";
                    } else {
                        echo '<li class="breadcrumb-item is-current">'. strip_tags( apply_filters( 'single_post_title', get_the_title( $ancestor ) ) ) .'</li>'."\n";
                    }
                }
            }
        }
